...
import { initialData } from "./initialData.js"; // Importing  initial data from initialData.js
...
function fetchAndDisplayBoardsAndTasks() {
  ...
  const boards = [
    ...new Set(tasks.map((task) => task.board).filter(Boolean)),
  ];
  displayBoards(boards);
  if (boards.length > 0) {
    const localStorageBoard = JSON.parse(
...
function filterAndDisplayTasksByBoard(boardName) {
  ...
  elements.columnDivs.forEach((column) => {
    ...
    filteredTasks
      .filter((task) => task.status === status) // Added === for comparison
      .forEach((task) => {
...
// Refresh the UI each time it is call in different functions for each modal
...
function setupEventListeners() {
  ...
  ); // Added an event listener for 'click'
